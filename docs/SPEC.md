...
**DC の状態を保存しない場合** — DIB と DC のあいだの転送（`BitBlt`）は論理座標で動くので、写像のかかった DC では、転送のあいだだけ DC をデバイス単位に切り替え、`SaveDC` / `RestoreDC` で戻す（`dib.rs` の `DeviceUnits`）。これを写像のかかっていない DC でもしていた。写像を変えると GDI は DC の実体化済みのフォントを捨てるので、次の描画の計測（`GetTextExtentPoint32W`）が約 22µs から約 210µs になっていた（実測）。いまは、DC がもともとデバイス単位なら（`MM_TEXT`、ビューポートとウィンドウの原点が 0、互換グラフィックスモードか単位行列のワールド変換）何も変えない。原点をずらした DC・2 倍に拡大した DC・`GM_ADVANCED` の DC では、変更の前後で描画結果がバイト単位で同じで、描いたあとの DC の状態も元のままだった。

実測（`verify/gdi-perf`、交互に 5 回の中央値、500 回あたり）: 約 402ms → 約 268ms。このときの upstream（`05052e8` を `Rel+Detours|x64` でビルド、同じプロファイル）は約 230ms、素の GDI は約 28ms。
