...
- 上流にない機構（監視スレッド・独自の設定ファイル・ライブ再読込など）を足さない。足すなら結果（スレッドの寿命・アンロード後に何が残るか・そのファイルを誰が書くか・失敗時にどのプロセスが落ちるか）を先に列挙する
...
## 裏取り
- 上流の挙動は [MacType の実コード](https://github.com/snowie2000/mactype/blob/05052e88c7ce134f93b66db95132284a1ed10de7)（移植元コミット `05052e8`）で確認する。推測で書かない
