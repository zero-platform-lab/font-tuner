...
//! (upstream MacType ft.cpp, commit 05052e8), in the same fixed-point integer
//! arithmetic so the output is bit-identical to upstream (same truncation at
//! the `>>16` decode step). The formula itself is written out in
//! docs/SPEC.md 2.3. An f32 port that rounds to nearest was tried and
//! reverted: it differs from upstream by one level, and in practice that was
//! visible as uneven ("bumpy") glyph edges.
//!
...
//! deliberate and match the C semantics; the regression tests below (values
//! captured from the C++ code) catch any drift. Hence the scoped cast allows.
...
mod tests {
    ...
    /// upstream C++ math, so this guards the LUT + blend against drift and
    /// must stay bit-exact.
    #[test]
