...
ブレンドの計算式は **docs/SPEC.md §2.3** を参照（ガンマ符号化・カバレッジ曲線・
線形空間ブレンド）。実装は上流と同じ固定小数点の整数なので、出力バイトは
上流の C++ と一致する。`cargo test`（`src/filter.rs`）で検証する: 端点、単調性、
全 `GammaMode` 分岐、上流の値を写した gamma 1.25 のグレースケール回帰（バイト
一致）。ブレンドテストは純粋なのでフォント不要。
